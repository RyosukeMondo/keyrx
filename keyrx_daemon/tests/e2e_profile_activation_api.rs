...
async fn test_profile_activation_does_not_block_config_page() {
    ...
    // let config_result = timeout(config_timeout, client.get("/api/profiles/default/config").send()).await;
...
async fn test_concurrent_profile_operations() {
    ...
    assert!(duration < Duration::from_millis(200), "Tasks should run concurrently");
...
async fn test_real_daemon_activation_freeze() {
    ...
    let config_result = timeout(
        ...
        client.get(&format!("{}/api/profiles/default/config", base_url)).send()
    ).await;
