...
async fn test_concurrent_read_write_mix() {
    ...
    println!("✓ 10 concurrent read/write mix completed in {:?}", duration);
}
...
async fn test_config_freeze_regression() {
    ...
    let config_result =
        tokio::time::timeout(config_timeout, app.get("/api/profiles/freeze-test/config")).await;
...
async fn test_concurrent_profile_activation_no_deadlock() {
    ...
    let handles: Vec<_> = (0..5)
        .map(|i| {
            ...
            tokio::spawn(async move {
                let response = client
                    .post(&format!("/api/profiles/profile-{}/activate", i), &json!({}))
                    .await;
                (i, response.status().as_u16())
            })
        })
...
async fn test_api_stress_1000_requests() {
    ...
    println!("  Throughput: {:.0} req/s", 1000.0 / duration.as_secs_f64());
}
...
async fn test_sustained_load() {
    ...
    println!("  Average wave time: {:?}", total_duration / 5);
}
