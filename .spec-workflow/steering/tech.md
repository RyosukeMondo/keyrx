...
### The Key Identity Pipeline (3-Stage)

The system transforms input in 3 stages to decouple hardware from logic, supporting **Multi-Device Concurrency**:

```
1. Physical Layer (Hardware)
   [Scancode 0x04] ("A" key on Device X)
         │
         ▼ (RuntimeConfig: Lookup Active Slots for Device X)
         │
2. Virtual Layer (Abstraction)
   [Priority 1 Slot] -> [Profile A] -> [Virtual Key: KEY_MACRO_1]
   [Priority 2 Slot] -> [Profile B] -> [Virtual Key: KEY_A]
         │
         ▼ (Conflict Resolution: Topmost slot consumes event)
         │
   [Virtual Key ID]
         │
         ▼ (Keymap: Mapped to Virtual Layout)
         │
3. Logical Layer (Host)
   [Logical Action] (e.g., "Output 'A'", "Macro 1", "Layer Toggle")
```

```mermaid
flowchart LR
    FW["Hardware\nscan_code (0x04)"] --> RUNTIME["RuntimeConfig\n(Active Slots)"]
    RUNTIME --> PRIORITY{Priority\nCheck}
    PRIORITY -->|Slot 1 Match| VIRT1["Virtual Layout\n(Slot 1)"]
    PRIORITY -->|Slot 2 Match| VIRT2["Virtual Layout\n(Slot 2)"]
    VIRT1 --> KMAP["Keymap\nLogical Mapping"]
    KMAP --> ACT["Action\nOutput/Macro"]

    style VIRT1 fill:#eef9ff,stroke:#5b8def
```

### Terminology

- **VirtualLayout**: Defines the available set of keys.
  - **Matrix Mode**: IDs are coordinates (`r0c0`). Good for hand-wired devices.
  - **Semantic Mode**: IDs are names (`KEY_A`). Good for standard keyboards.
- **HardwareProfile**: Defines **Hardware Wiring** (Physical Scancode -> Virtual Key ID).
- **Keymap**: Defines **Logical Mapping** (Virtual Key ID -> Logical Action).
- **Profile Slot**: A runtime assignment of (HardwareProfile + Keymap) to a physical device.
- **RuntimeConfig**: The active state of all devices and their slots.

### Core Data Structures

```rust
/// Defines the set of available virtual keys (Matrix or Semantic)
pub struct VirtualLayout {
    pub id: String,
    pub name: String,
    /// Type: "Matrix" (rows/cols) or "Semantic" (named keys)
    pub layout_type: LayoutType,
    /// Definition of keys available in this layout
    pub keys: Vec<VirtualKeyDef>,
}

/// Hardware wiring: Physical Scancode -> Virtual Key
pub struct HardwareProfile {
    pub vendor_id: u16,
    pub product_id: u16,
    pub name: String,
    /// Link to the Virtual Layout this hardware uses
    pub virtual_layout_id: String,
    /// Wiring: Scancode -> Virtual Key ID
    pub wiring: HashMap<u16, String>,
}

/// Logical Mapping: Virtual Key -> Actions
pub struct Keymap {
    /// Link to the Virtual Layout being mapped
    pub virtual_layout_id: String,
    /// Mapping: Virtual Key ID -> Logical Action
...
pub struct DeviceState {
    pub active: bool,
    /// Ordered list of active profile slots (Higher index = Higher priority)
...
```
~/.config/keyrx/
├── layouts/               # Reusable Virtual Layouts
│   ├── ansi_104.json
│   └── stream_deck_4x4.json
├── hardware/              # Hardware Profiles (Wiring)
│   ├── 046d_c52b.json     # Logitech K270 (Wired to ansi_104)
│   └── 1234_5678.json     # Custom Pad (Wired to stream_deck_4x4)
└── keymaps/               # Logical Mappings
    └── my_gaming_layer.json
```

### Modifier Keys as Pure Buttons

At the hardware level, modifier keys are identical to regular keys. They are just buttons in the `HardwareProfile` that get mapped to a `Virtual Key` (e.g., `KEY_LSHIFT`). The `Keymap` then decides if `KEY_LSHIFT` acts as a modifier or something else.
