...
/**
 * Fetch profile configuration source code via REST API with React Query caching.
 *
 * Uses REST instead of WebSocket RPC so config loads reliably on page reload
 * without waiting for WebSocket handshake.
 *
...
export function useGetProfileConfig(name: string) {
  return useQuery({
    ...
    queryFn: () => fetchProfileConfig(name),
