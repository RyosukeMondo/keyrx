...
import { type LayoutType, type LayoutDropdownOption, LAYOUT_OPTIONS } from '../contexts/LayoutPreviewContext';
...
export const LayoutDropdown = React.memo<LayoutDropdownProps>(({
  ...
  const handleOptionHover = useCallback((optionValue: LayoutType) => {
  ...
  const setOptionRef = useCallback((value: string) => (el: HTMLDivElement | null) => {
    if (el) {
          ...
          <div className={`relative ${className}`}>
            <ListboxButton
              ...
              className={`
                ...
                ${compact ? 'py-1.5 px-3 text-sm min-w-[140px]' : 'w-full py-3 px-4 text-base'}
              `}
            >
                  ...
                  <ListboxOption
                    ...
                    className={({ focus, selected }) =>
                      `
                        ...
                        ${compact ? 'py-1.5 px-3 text-sm' : 'py-2 px-3 text-base'}
