...
use super::builtins::{
    ...
    parse_layer_action, validate_timeout, with_layer_view, with_modifier_view, LayerMapAction,
    LayerView, ModifierView, PendingOp, PendingOps, TimingUpdate,
};
...
pub fn register_all_functions(
    ...
    register_layout_functions(engine, pending_ops);
    ...
    register_remap_rc(engine, pending_ops, resolver);
    register_tap_hold_rc(engine, pending_ops, resolver);
    register_block_rc(engine, pending_ops, resolver);
    register_combo_rc(engine, pending_ops, resolver);
    register_layer_map_rc(engine, pending_ops, resolver, layer_view);
}
...
fn register_layout_functions(engine: &mut Engine, pending_ops: &PendingOps) {
...
fn combo_rc_impl(
    ...
    if let Ok(mut ops) = ops.lock() {
        ops.push(PendingOp::Combo {
            keys: parsed_keys,
            action: crate::engine::LayerAction::Remap(action),
        });
    }
