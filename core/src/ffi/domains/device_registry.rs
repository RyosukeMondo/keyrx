...
pub extern "C" fn keyrx_device_registry_list_devices() -> *mut c_char {
    tracing::debug!("keyrx_device_registry_list_devices called");
    std::panic::catch_unwind(|| {
        let result: FfiResult<Vec<FfiDeviceState>> = with_revolutionary_runtime(|runtime| {
            tracing::debug!("keyrx_device_registry_list_devices: runtime acquired");
        ...
        if let Ok(ref res) = result {
            ...
            tracing::debug!("keyrx_device_registry_list_devices: returning error");
        }
        ffi_response(result)
    })
    .unwrap_or_else(|_| {
        tracing::error!("keyrx_device_registry_list_devices: PANIC caught");
