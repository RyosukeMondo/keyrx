...
impl ReportGenerator {
    ...
    pub fn generate_markdown(&self, data: &ReportData) -> String {
        ...
        // Summary section
        md.push_str(&self.markdown_summary_section(data));

        // Quality gate section (if available)
        if let Some(ref gate_result) = data.gate_result {
            md.push_str(&self.markdown_gate_section(gate_result));
        }

        // Failed tests section
        md.push_str(&self.markdown_failed_tests_section(data));

        // Results by category
        md.push_str(&self.markdown_category_section(data));

        // Results by priority
        md.push_str(&self.markdown_priority_section(data));

        // Coverage section (if available)
        if let Some(ref coverage) = data.coverage {
            md.push_str(&self.markdown_coverage_section(coverage));
        }

        // Performance section (if available)
        if let Some(ref perf) = data.performance {
            md.push_str(&self.markdown_performance_section(perf));
        }
    ...
    fn markdown_summary_section(&self, data: &ReportData) -> String {
    ...
    fn markdown_gate_section(&self, gate_result: &GateResult) -> String {
    ...
    fn markdown_failed_tests_section(&self, data: &ReportData) -> String {
    ...
    fn markdown_category_section(&self, data: &ReportData) -> String {
    ...
    fn markdown_priority_section(&self, data: &ReportData) -> String {
    ...
    fn markdown_coverage_section(&self, coverage: &CoverageReport) -> String {
        let mut md = format!(
    ...
    fn markdown_performance_section(&self, perf: &PerfResults) -> String {
        let mut md = format!(
            "## Performance Metrics\n\n\
            | Percentile | Latency |\n\
            |------------|--------|\n\
