...
mod tests {
    ...
    async fn test_device_service_lifecycle() {
        ...
        let service = DeviceService::new(None).with_bindings_path(bindings_path.clone());
        ...
        // 2. Assign profile
        let view = service.assign_profile(device_key, "profile-1").await.expect("assign");
        ...
        let service2 = DeviceService::new(None).with_bindings_path(bindings_path.clone());
        let view2 = service2.get_device(device_key).await.expect("get_device 2");
        assert_eq!(view2.profile_id.as_deref(), Some("profile-1"));

        // 3. Remap
        let view = service.set_remap_enabled(device_key, false).await.expect("remap");
        assert!(!view.remap_enabled);

        // 4. Label
        let view = service.set_label(device_key, Some("My Keeb".to_string())).await.expect("label");
        assert_eq!(view.label.as_deref(), Some("My Keeb"));

        // 5. Unassign
        let view = service.unassign_profile(device_key).await.expect("unassign");
        assert!(view.profile_id.is_none());
    }
}
